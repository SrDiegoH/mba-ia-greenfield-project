...
- [ ] T056 Validar Definition of Done: `npx tsc --noEmit` (sem erros de TypeScript), `npm run lint` (sem warnings), `npm test` (unit + integration passando), `npm run test:e2e` (E2E passando com `--runInBand`), `npm run migration:run` não tem migrações pendentes — **REQUER CONTAINERS: executar dentro do container após `docker compose up -d`**
